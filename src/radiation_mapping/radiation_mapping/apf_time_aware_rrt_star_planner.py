...
class APFTimeAwareRRTStarPlanner(Node):
    def __init__(self):
        super().__init__('apf_time_aware_rrt_star_planner')
        ...
        self.marker_pub = self.create_publisher(
            MarkerArray,
            'apf_time_aware_rrt_star_markers',
            10
        )

        self.timer = self.create_timer(1.0, self.timer_callback)

        self.get_logger().info('APF-guided time-aware RRT* planner started.')
    ...
    def get_risk_repulsive_direction(self, x, y):
        ...
        # Move opposite to the risk gradient
        repulsive_x = -grad_x
        repulsive_y = -grad_y

        return self.normalize_vector(repulsive_x, repulsive_y)

    def apply_apf_guidance(self, current_node, random_node):
        # Random exploration direction
        random_dx = random_node.x - current_node.x
        random_dy = random_node.y - current_node.y
        random_dx, random_dy = self.normalize_vector(random_dx, random_dy)

        # Goal attractive direction
    ...
    def create_tree_marker(self):
        ...
        marker.ns = 'apf_time_aware_rrt_star_tree'
        marker.id = 0
    ...
    def create_final_path_marker(self):
        ...
        marker.ns = 'apf_time_aware_rrt_star_path'
        marker.id = 1
    ...
    def create_text_marker(self):
        ...
        marker.ns = 'apf_time_aware_rrt_star_label'
        marker.id = 2
        marker.type = Marker.TEXT_VIEW_FACING
        marker.action = Marker.ADD

        marker.text = 'APF-Time-Aware RRT*'
...
def main(args=None):
    ...
    node = APFTimeAwareRRTStarPlanner()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()

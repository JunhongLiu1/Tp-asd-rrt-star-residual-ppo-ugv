...
def create_full_resampled_dem(
    ...
    """
    Resample the COMPLETE source DEM to samples × samples.

    This operation does not crop the source area. The complete source
    extent is retained and represented by a smaller raster.
    """
    ...
    # Try to fill small NODATA regions using nearby elevation values.
    try:
        gdal.FillNodata(
    ...
    if not np.any(valid):
        raise RuntimeError(
            'The complete resampled DEM contains no valid cells.'
        )
    ...
    elevation[~valid] = fill_value

    # Move the lowest point of the complete terrain to z = 0.
    ...
    output_band.SetDescription(
        'Complete zero-based SnowEx elevation'
    )

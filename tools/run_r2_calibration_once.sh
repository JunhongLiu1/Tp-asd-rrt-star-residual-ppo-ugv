...
MAP_CAPTURE="${WS}/tools/capture_radiation_map_stats_v4.py"
SCENARIO="${1:-H_S1_R2_final}"
SEED="${2:-31}"
STAMP="$(date +%Y%m%d_%H%M%S)"
CALIBRATION_ID="r2_calibration_${SCENARIO}_s${SEED}_${STAMP}"
OUT="${WS}/final_score_calibration_r2/${CALIBRATION_ID}"
...
run_one() {
    local planner="$1"
    local capture_map="$2"
    local run_id="${CALIBRATION_ID}_${planner}"
    local code=0

    echo
    echo "========================================================================"
    echo "R2 PILOT CALIBRATION: scenario=${SCENARIO} planner=${planner} seed=${SEED}"
    echo "========================================================================"
    ...
    if [ "${capture_map}" = "yes" ]; then
        ...
        if [ "${capture_code}" -ne 0 ]; then
            echo "[WARN] Radiation-map stats capture failed with code ${capture_code}"
        fi
    else
        set +e
        "${RUNNER}" \
...
echo
echo "Pilot complete. Candidate references are NOT frozen yet."
echo "Output: ${OUT}"

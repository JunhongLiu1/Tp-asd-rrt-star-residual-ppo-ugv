...
TP_CONFIG="$WS/src/radiation_mapping/config/final_cost_model_v1.json"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
RESULT_DIR="$WS/results/final_asd_tp_comparison/$RUN_ID"

ASD_CSV="$RESULT_DIR/asd_rrt_star_hard.csv"
TP_CSV="$RESULT_DIR/tp_asd_rrt_star_hard.csv"
...
launch_terminal \
    "04 TP-ASD-RRT Star Lambda 0.25" \
...
launch_terminal \
    "06 TP Path Evaluator Lambda 0.25" \
    "ros2 run radiation_mapping terrain_path_evaluator \
        --ros-args \
        ...
        -p radiation_topic:=/radiation_map \
        -p planner_name:=tp_asd_rrt_star \
